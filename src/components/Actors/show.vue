<script>
  import getActor from "./composables/getActor";
  import getActorExternals from "./composables/getActorExternals";
  import getActorCombinedCredits from "./composables/getActorCombinedCredits";
  import { onMounted } from "vue";
  import getActorAge from "./composables/getActorAge";
  import sliceArray from "../../composables/sliceArray";
  import getActorImages from "./composables/getActorImages";

  export default {
    props: ["id"],
    setup(props) {
      const { actor, loadActor } = getActor();
      const { social, loadActorExternals } = getActorExternals();
      const { actorCombinedCredits, loadActorCombinedCredits } =
        getActorCombinedCredits();
      const { actorImages, loadActorImages } = getActorImages();

      onMounted(() => {
        loadActor(props.id);
        loadActorExternals(props.id);
        loadActorCombinedCredits(props.id);
        loadActorImages(props.id);
      });
...
<template>
    ...
    <div class="container flex flex-col px-4 py-16 mx-auto md:flex-row">
      <div class="flex-none">
        ...
        <ul class="flex items-center mt-4">
          ...
          <li class="ml-6" v-if="social['instagram_id']">
            <a
          ...
          <li class="ml-6" v-if="social['twitter_id']">
            <a
          ...
          <li class="ml-6" v-if="social['homepage']">
      ...
      <div class="md:ml-24">
        ...
        <div class="flex flex-wrap items-center text-sm text-gray-400">
          <svg
            class="w-4 text-gray-400 fill-current hover:text-white"
            viewBox="0 0 448 512"
          >
            <path
          ...
          <span class="ml-2"
            >{{ actor["birthday"] }} ({{ getActorAge(actor) }} years old) in
        ...
        <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-5">
          <div
            class="mt-4"
            v-for="actorCast in sliceArray(actorCombinedCredits.cast, 0, 5)"
            :key="actorCast.id"
          >
            <router-link
              v-if="actorCast['title']"
              :to="{ name: 'MoviesShow', params: { id: actorCast['id'] } }"
            >
              <img
                v-if="actorCast['poster_path']"
                :src="`https://image.tmdb.org/t/p/w400${actorCast['poster_path']}`"
                alt="poster"
                class="transition duration-150 ease-in-out hover:opacity-75"
              />
              <img
                v-else
                src="https://via.placeholder.com/300x430"
                alt="poster"
                class="transition duration-150 ease-in-out hover:opacity-75"
              />
              <span
                class="block mt-1 text-sm leading-normal text-gray-400 hover:text-white"
                >{{ actorCast["title"] }}</span
              >
            </router-link>
            <router-link
              v-if="actorCast['name']"
              :to="{ name: 'TvShow', params: { id: actorCast['id'] } }"
            >
              <img
                :src="`https://image.tmdb.org/t/p/w400${actorCast['poster_path']}`"
                alt="poster"
  ...
  <div class="flex">
    <img v-for="actorImage in actorImages" :key="actorImage"
      :src="`https://image.tmdb.org/t/p/w300${actorImage.file_path}`"
      alt="poster"
      class="transition duration-150 ease-in-out hover:opacity-75"
    />
  </div>
